...
export async function getPaymentUtxos(address) {
        try {
                ...
                throw new Error('Failed to get payment utxos: ' + error.message);
        }
}
...
export async function broadcastTransaction(psbtHex) {
        try {
                ...
                releaseUtxos(psbt.txInputs);  // Release UTXOs after successful broadcast
                ...
                releaseUtxos(psbt.txInputs);  // Release UTXOs if broadcast fails
                return {
                        success: false,
